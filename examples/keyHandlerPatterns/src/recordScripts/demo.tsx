...
import {Box, Button, FillBox, KeyEvent, UILayer} from "@launchmenu/core";
...
export default declareVideoScript(
    async ({controller, recorder, visualizer, keyVisualizer, LM}) => {
        ...
        try {
            ...
            await controller.wait(1000);
            await controller.press(["ctrl", "f"]);
            await controller.wait(2500);
            await controller.press("enter");

            await controller.wait(1000);
            await controller.press(["ctrl", "alt", "f"]);
            await controller.wait(2500);
            await controller.press("enter");

            await controller.wait(1000);
            await controller.hold(["ctrl", "shift"], {
                whileHeld: async () => {
                    const key = {id: "keyD", name: "d", char: "D"} as const;
                    await controller.press(new KeyEvent({key, type: "down"}));
                    await controller.wait(200);
                    await controller.press(new KeyEvent({key, type: "up"}));
                },
            });
            await controller.wait(2500);
            await controller.press("enter");
