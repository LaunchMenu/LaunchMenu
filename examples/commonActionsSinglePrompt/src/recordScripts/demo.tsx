...
export default declareVideoScript(
    async ({controller, recorder, visualizer, keyVisualizer, LM}) => {
        ...
        await controller.wait(1000);
        await controller.type([{key: "down", repeat: 3, repeatDelay: 1500}]);
        await controller.wait(1000);
        await controller.hold(["shift"], {
            whileHeld: async () => {
                await controller.type([{key: "up", repeat: 2, repeatDelay: 500}]);
            },
        });
        await controller.press("enter");
        await controller.wait(1500);
        await controller.type([{text: "270"}, {delay: 1000, key: "enter"}]);
        await controller.wait(1000);
        await controller.type([{key: "down", repeat: 3, repeatDelay: 1500}]);
