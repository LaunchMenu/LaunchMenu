...
export class SettingsManager {
    protected files = new Field([] as ISettingsData<IJSONDeserializer>[]);
    ...
    public getSettingsData(
        ...
        return this.files.get(hook).find(({ID: OID}) => OID == ID) ?? null;
    }
    ...
    public updateSettings(
        ...
        const files = this.files.get(null);
        this.files.set([
    ...
    public removeSettings(ID: IUUID): void {
        const files = this.files.get(null);
        this.files.set(files.filter(({ID: OID}) => OID != ID));
    }
    ...
    public async reloadAll(): Promise<void> {
        await Promise.all(this.files.get(null).map(({file}) => file.load()));
    }
    ...
    public async saveAll(): Promise<void> {
        ...
        await Promise.all(this.files.get(null).map(({file}) => file.save()));
    }
}
