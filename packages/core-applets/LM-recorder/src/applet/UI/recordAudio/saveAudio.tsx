...
import {DataCacher, DataLoader, Field, IDataHook, IDataRetriever} from "model-react";
...
export class AudioSaverLayer extends UILayer {
    ...
    protected directory?: IDataRetriever<string>;
    ...
    public constructor({recorder}: {recorder: AudioRecorder}) {
        super(
            context => {
                this.directory = h => context.settings.get(settings).audioDir.get(h);

                const customItem = this.getCustomView();
                const menu = new ProxiedMenu(context, h => [
                    ...this.replaceItems.get(h),
                    customItem,
                ]);
    ...
    /** The source for items to be replaced */
    protected dirContentsSource = new DataCacher<DataLoader<string[]>>(h => {
        return new DataLoader<string[]>(async () => {
            const dirPath = this.directory?.(h);
            const files = dirPath ? await FS.readdir(dirPath) : [];
            return files;
        }, []);
    });

    /** The data source to get the items to be replaced */
    protected replaceItems = new DataCacher<IMenuItem[]>(h => {
        const files = this.dirContentsSource.get(h).get(h);
        try {
